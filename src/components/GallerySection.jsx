      ...
      <div className="md:py-40 py-20 md:px-40 bg-white">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 place-items-center px-3" >
          {images.slice(0, numbersToShow).map((img, i) => (
            <div key={i} className="w-full h-full" data-aos="fade-up" data-aos-duration="1000">
              <img src={img} alt="Gallery" className="rounded-lg shadow w-100 h-full" />

            </div>))}
        </div>
        <div className="text-center mt-10 ">
          {
            location.pathname !== "/gallery" && <Link to="/gallery" ><button className="px-4 py-2  text-white rounded hover:bg-blue-900 cursor-pointer" style={{ backgroundColor: '#273359' }} data-aos="fade-up" data-aos-duration="1000">
              View More
            </button></Link>
          }
        </div>
